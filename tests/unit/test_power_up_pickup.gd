...
func test_ale_pickup_does_not_increment_mushrooms_consumed_counter():
	var pickup := _make_pickup(PowerUpEffect.TYPE_ALE)
	var p := _make_player()
	pickup._on_body_entered(p)
	assert_eq(int(GameState.achievement_service.account.achievement_counters.get("mushrooms_consumed", 0)), 0,
		"an ale pickup must not touch the mushrooms_consumed counter")


func test_catnip_pickup_does_not_increment_mushrooms_consumed_counter():
	var pickup := _make_pickup(PowerUpEffect.TYPE_CATNIP)
	var p := _make_player()
	pickup._on_body_entered(p)
	assert_eq(int(GameState.achievement_service.account.achievement_counters.get("mushrooms_consumed", 0)), 0,
		"a catnip pickup must not touch the mushrooms_consumed counter")

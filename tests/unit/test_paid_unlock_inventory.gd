...
func test_grant_then_has_unlock():
	...
	assert_true(inv.grant("wizard_cat"))
	assert_true(inv.has_unlock("wizard_cat"))

func test_grant_is_idempotent():
	var inv := PaidUnlockInventory.new()
	assert_true(inv.grant("wizard_cat"))
	assert_false(inv.grant("wizard_cat"), "second grant returns false")
	assert_eq(inv.owned_class_ids.size(), 1, "no duplicate entries")

func test_has_unlock_false_for_unowned():
	var inv := PaidUnlockInventory.new()
	assert_false(inv.has_unlock("wizard_cat"))

func test_grant_normalizes_case():
	# Class id keys are lower-cased on write so a case-mismatched product id
	# parse (e.g. "Chonk_Kitten") doesn't produce a parallel entry next
	# to "chonk_kitten". Matches UnlockRegistry's case-insensitive lookup shape.
	var inv := PaidUnlockInventory.new()
	assert_true(inv.grant("Chonk_Kitten"))
	assert_true(inv.has_unlock("chonk_kitten"), "lookup case-insensitive")
	assert_false(inv.grant("CHONK_KITTEN"), "case-folded duplicate rejected")
...
func test_dict_round_trip():
	var inv := PaidUnlockInventory.new()
	inv.grant("wizard_cat")
	inv.grant("battle_cat")
	var restored := PaidUnlockInventory.from_dict(inv.to_dict())
	assert_true(restored.has_unlock("wizard_cat"))
	assert_true(restored.has_unlock("battle_cat"))
...
func test_from_dict_skips_empty_and_dedupes():
	...
	var restored := PaidUnlockInventory.from_dict({
		"owned_class_ids": ["wizard_cat", "", "WIZARD_CAT", "sleepy_kitten"],
	})
	assert_eq(restored.owned_class_ids.size(), 2)
	assert_true(restored.has_unlock("wizard_cat"))
	assert_true(restored.has_unlock("sleepy_kitten"))
...
func test_to_dict_clones_array():
	...
	inv.grant("wizard_cat")
	var d := inv.to_dict()
	d["owned_class_ids"].append("sleepy_kitten")
	assert_false(inv.has_unlock("sleepy_kitten"), "inventory unaffected")

# --- KittenSaveData wiring --------------------------------------------------

func test_kitten_save_data_to_dict_emits_paid_class_unlocks():
	var sd := KittenSaveData.new()
	sd.paid_class_unlocks = ["wizard_cat"]
	var d := sd.to_dict()
	assert_true(d.has("paid_class_unlocks"))
	assert_eq(d["paid_class_unlocks"], ["wizard_cat"])

func test_kitten_save_data_paid_class_unlocks_round_trips():
	var sd := KittenSaveData.new()
	sd.paid_class_unlocks = ["wizard_cat", "battle_cat"]
	var restored := KittenSaveData.from_dict(sd.to_dict())
	assert_eq(restored.paid_class_unlocks.size(), 2)
	assert_true(restored.paid_class_unlocks.has("wizard_cat"))
	assert_true(restored.paid_class_unlocks.has("battle_cat"))
...
func test_kitten_save_data_from_character_captures_paid_unlocks():
	...
	var inv := PaidUnlockInventory.new()
	inv.grant("wizard_cat")
	inv.grant("sleepy_kitten")
	var sd := KittenSaveData.from_character(c, null, null, null, null, inv)
	assert_eq(sd.paid_class_unlocks.size(), 2)
	assert_true(sd.paid_class_unlocks.has("wizard_cat"))
	assert_true(sd.paid_class_unlocks.has("sleepy_kitten"))
...
func test_kitten_save_data_to_paid_unlock_inventory():
	...
	assert_true(inv.has_unlock("wizard_cat"))
	assert_false(inv.has_unlock("sleepy_kitten"))
...
func test_is_unlocked_paid_bypasses_threshold():
	...
	assert_false(registry.is_unlocked("wizard_cat", tracker, paid),
		"locked before purchase (no condition met, no paid entry)")
	paid.grant("wizard_cat")
	assert_true(registry.is_unlocked("wizard_cat", tracker, paid),
...
func test_is_unlocked_paid_works_with_null_tracker():
	...
	var paid := PaidUnlockInventory.new()
	paid.grant("wizard_cat")
	assert_true(registry.is_unlocked("wizard_cat", null, paid),
...
func test_class_id_for_unlock_chonk_kitten():
	assert_eq(
		PurchaseRegistry.class_id_for_unlock(PurchaseRegistry.CLASS_UNLOCK_CHONK_KITTEN),
		"chonk_kitten")
...
func test_handle_class_unlock_grants_paid_entry():
	...
	# PurchaseGrantHandler routes to PaidUnlockInventory.grant("wizard_cat") ->
...
func test_handle_class_unlock_does_not_mutate_character():
	...
	PurchaseGrantHandler.handle(
		PurchaseRegistry.CLASS_UNLOCK_CHONK_KITTEN, c, cos, paid)
	assert_eq(c.character_class, int(CharacterData.CharacterClass.WIZARD_KITTEN),
...
func test_game_state_apply_merged_save_hydrates_paid_unlocks():
	...
	save.paid_class_unlocks = ["wizard_cat"]
	gs.apply_merged_save(save)
	assert_true(gs.paid_unlocks.has_unlock("wizard_cat"))
	assert_false(gs.paid_unlocks.has_unlock("sleepy_kitten"))
...
func test_purchase_succeeded_signal_grants_paid_unlock():
	...
	bm.purchase_succeeded.emit(PurchaseRegistry.CLASS_UNLOCK_CHONK_KITTEN)
	assert_true(gs.paid_unlocks.has_unlock("chonk_kitten"),
...
func test_purchase_succeeded_persists_paid_unlock_to_save():
	...
	assert_true(loaded.paid_class_unlocks.has("chonk_kitten"),
...
func test_unlock_registry_reads_paid_unlocks_from_game_state():
	...
	var gs := get_node("/root/GameState")
	gs.paid_unlocks.grant("sleepy_kitten")
	var unlocked: bool = gs.unlock_registry.is_unlocked(
		"sleepy_kitten", gs.meta_tracker, gs.paid_unlocks)

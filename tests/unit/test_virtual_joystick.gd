...
func test_compute_clamped_offset_within_radius_unchanged():
	...
	var offset := KittenJoystick.compute_clamped_offset(touch, center, BASE)
...
func test_compute_clamped_offset_at_center_is_zero():
	var center := Vector2(50, 200)
	var offset := KittenJoystick.compute_clamped_offset(center, center, BASE)
...
func test_compute_direction_zero_within_deadzone():
	...
	var dir := KittenJoystick.compute_direction(offset, BASE, DEADZONE)
...
func test_compute_direction_outside_deadzone_normalized_by_radius():
	...
	var dir := KittenJoystick.compute_direction(Vector2(BASE * 0.5, 0), BASE, DEADZONE)
...
func test_compute_direction_at_max_offset_is_unit_vector():
	var dir := KittenJoystick.compute_direction(Vector2(BASE, 0), BASE, DEADZONE)
...
func test_compute_direction_zero_radius_safe():
	...
	var dir := KittenJoystick.compute_direction(Vector2(10, 10), 0.0, DEADZONE)
	assert_eq(dir, Vector2.ZERO,
...
func test_compute_action_strengths_right_only_fills_right():
	var s := KittenJoystick.compute_action_strengths(Vector2(0.7, 0.0))
...
func test_compute_action_strengths_left_only_fills_left():
	var s := KittenJoystick.compute_action_strengths(Vector2(-0.5, 0.0))
...
func test_compute_action_strengths_zero_input_all_zero():
	var s := KittenJoystick.compute_action_strengths(Vector2.ZERO)
	assert_eq(float(s["move_left"]), 0.0)
	assert_eq(float(s["move_right"]), 0.0)
	assert_eq(float(s["move_up"]), 0.0)
	assert_eq(float(s["move_down"]), 0.0)
...
func test_pipeline_right_edge_touch_drives_move_right_only():
	...
	var off := KittenJoystick.compute_clamped_offset(touch, center, BASE)
	var dir := KittenJoystick.compute_direction(off, BASE, DEADZONE)
	var s := KittenJoystick.compute_action_strengths(dir)
...
func test_pipeline_dead_center_yields_no_strength():
	...
	var center := Vector2(50, 200)
	var off := KittenJoystick.compute_clamped_offset(center, center, BASE)
	var dir := KittenJoystick.compute_direction(off, BASE, DEADZONE)
	var s := KittenJoystick.compute_action_strengths(dir)
...
func test_reset_clears_captured_touch_and_thumb_offset():
	var joystick: KittenJoystick = add_child_autofree(KittenJoystick.new())

...
func test_scene_has_required_nodes():
	var s := _instantiate()
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/Headline"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/Stats/FloorLabel"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/Stats/EnemiesLabel"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/Stats/XPLabel"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/Stats/GoldLabel"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/ButtonRow/NextFloor"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/ButtonRow/UpdateCharacter"))
	assert_not_null(s.get_node_or_null("Backdrop/Center/Panel/Scroll/VBox/ButtonRow/SaveAndExit"))
...
func test_update_character_pressed_fires_on_button_press():
	var s := _instantiate()
	watch_signals(s)
	var btn: Button = s.get_node("Backdrop/Center/Panel/Scroll/VBox/ButtonRow/UpdateCharacter")
	btn.pressed.emit()
...
func test_save_and_exit_pressed_fires_on_button_press():
	var s := _instantiate()
	watch_signals(s)
	var btn: Button = s.get_node("Backdrop/Center/Panel/Scroll/VBox/ButtonRow/SaveAndExit")
	btn.pressed.emit()
...
func test_headline_and_waiting_label_wrap_and_button_row_fits_narrow_viewport():
	...
	await get_tree().process_frame
	var headline: Label = s.get_node("Backdrop/Center/Panel/Scroll/VBox/Headline")
	var waiting_label: Label = s.get_node("Backdrop/Center/Panel/Scroll/VBox/ButtonRow/WaitingLabel")
	...
	var panel: Control = s.get_node("Backdrop/Center/Panel")
	var button_row: Control = s.get_node("Backdrop/Center/Panel/Scroll/VBox/ButtonRow")
...
# --- boss with a long display name: headline wraps to extra lines and must
# not push the panel taller than the viewport (screen got cut off, buttons
# unreachable). VBox sits inside a ScrollContainer that gets capped to the
# available viewport height once content no longer fits.

const SHORT_VIEWPORT_HEIGHT := 320.0

func test_panel_fits_short_viewport_when_long_headline_wraps():
	var s := _instantiate()
	s.populate(FloorRunSummary.new(5, 41, 2404, 9518),
		"Congratulations on vanquishing Big Bruiser Buster!")
	await get_tree().process_frame
	await get_tree().process_frame
	s._update_panel_width(NARROW_VIEWPORT_WIDTH)
	await get_tree().process_frame
	await get_tree().process_frame
	s._update_panel_height(SHORT_VIEWPORT_HEIGHT)
	await get_tree().process_frame
	await get_tree().process_frame
	var panel: Control = s.get_node("Backdrop/Center/Panel")
	assert_lte(panel.size.y, SHORT_VIEWPORT_HEIGHT,
		"Panel height must not exceed a short viewport even when the headline wraps to extra lines")
